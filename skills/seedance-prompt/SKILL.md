...
6. Give every camera move a visible or dramatic reason; if the move adds no information or emotional change, simplify it.
...
`起始景别与首帧构图 + 主体屏幕位置/深度/比例 + 前中后景与负空间 + 继承的光学关系或一个焦段/FOV + 角度/高度/轴线 + 运镜及动机 + 速度 + 稳定方式 + 焦点 + 必要的结束景别`
